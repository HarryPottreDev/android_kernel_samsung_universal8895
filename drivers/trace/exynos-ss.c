...
int exynos_ss_post_panic(void)
{
	if (ess_base.enabled) {
		...
		if (sec_debug_get_debug_level() &&
			(__raw_readl(exynos_ss_get_base_vaddr() + ESS_OFFSET_SCRATCH)
				== ESS_SIGN_SCRATCH))
...
void exynos_ss_spinlock(void *v_lock, int en)
{
		...
		return;
	{
		...
		unsigned long i = index & (ARRAY_SIZE(ess_log->spinlock[0]) - 1);
#ifdef CONFIG_DEBUG_SPINLOCK
...
void exynos_ss_irqs_disabled(unsigned long flags)
{
	...
	if (unlikely(flags)) {
		...
		unsigned long i = index % ARRAY_SIZE(ess_log->irqs_disabled[0]);
...
void exynos_ss_printk(const char *fmt, ...)
{
		...
		return;
	{
		int cpu = get_current_cpunum();
		va_list args;
		unsigned long i = atomic_inc_return(&ess_idx.printk_log_idx) &
...
void exynos_ss_printkl(size_t msg, size_t val)
{
	...
	{
		int cpu = get_current_cpunum();
		unsigned long i = atomic_inc_return(&ess_idx.printkl_log_idx) &
